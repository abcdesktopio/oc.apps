...
## JSON dump
json source file klickety.d..json 
...
``` sh
ABCHOST=localhost
curl --output klickety.d..json https://raw.githubusercontent.com/abcdesktopio/oc.apps/main/klickety.d..json
curl -X PUT -H 'Content-Type: text/javascript' http://$ABCHOST:30443/API/manager/image -d @klickety.d..json
...
# Dockerfile Klickety is generated at Thu Mar 28 2024 15:35:00 GMT+0000 (Coordinated Universal Time)
...
RUN DEBIAN_FRONTEND=noninteractive apt-get update && apt-get install -y  --no-install-recommends breeze-icon-theme dbus-x11 dbus-user-session klickety && apt-get clean && rm -rf /var/lib/apt/lists/* 
...
``` sh
docker build --build-arg TAG= -f Klickety.d -t Klickety .
```
