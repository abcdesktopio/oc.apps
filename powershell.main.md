# powershell
![powershell.svg](icons/powershell.svg){: style="height:64px;width:64px"}
## inherite from
[abcdesktopio/oc.template.alpine](../abcdesktopio/oc.template.alpine)
## abcdesktop release
main
## Platforms
linux/amd64
## Distribution
alpine ![alpine](icons/alpine.svg){: style="height:32px;"}

``` 
NAME="Alpine Linux"
ID=alpine
VERSION_ID=3.18.3
PRETTY_NAME="Alpine Linux v3.18"
HOME_URL="https://alpinelinux.org/"
BUG_REPORT_URL="https://gitlab.alpinelinux.org/alpine/aports/-/issues"

```


## Alpine packages

``` 
powershell konsole font-adobe-source-code-pro
```

## Arguments
`"--name powershell -e /usr/bin/pwsh"`

## Displayname


``` 
Powershell
```

## Path


``` 
/usr/bin/konsole
```

## File extensions
`"ps1;"`
## ACL

``` json
{
    "permit": [
        "all"
    ]
}
```

## WM_CLASS

``` 
powershell.konsole
```

> The WM_CLASS property (of type STRING without control characters) contains two consecutive null-terminated strings. These specify the Instance and Class names to be used by both the client and the window manager for looking up resources for the application or as identifying information.
> to get the WM_CLASS property of an application, use the command line `wmctrl -lx`



## JSON dump
json source file powershell.d.main.json 

``` json
{
    "acl": {
        "permit": [
            "all"
        ]
    },
    "cat": "utilities",
    "platforms": "linux/amd64",
    "apkpackage": "powershell konsole font-adobe-source-code-pro",
    "icon": "powershell.svg",
    "keyword": "powershell",
    "launch": "powershell.konsole",
    "name": "powershell",
    "displayname": "Powershell",
    "rules": {
        "homedir": {
            "default": true
        }
    },
    "fileextensions": "ps1;",
    "path": "/usr/bin/konsole",
    "args": "--name powershell -e /usr/bin/pwsh",
    "template": "abcdesktopio/oc.template.alpine"
}
```

## Install the builded image
>Replace the **ABCHOST** var set to localhost by default to your own server ip address

``` sh
ABCHOST=localhost
curl --output powershell.d.main.json https://raw.githubusercontent.com/abcdesktopio/oc.apps/main/powershell.d.main.json
curl -X PUT -H 'Content-Type: text/javascript' http://$ABCHOST:30443/API/manager/image -d @powershell.d.main.json

```

## Generated `DockerFile` source code

``` 
# Dynamic DockerFile application file for abcdesktop.io generated by https://github.com/abcdesktopio/abcdesktopio/oc.apps/make.js
# DO NOT EDIT THIS FILE BY HAND -- YOUR CHANGES WILL BE OVERWRITTEN
# Dockerfile powershell is generated at Thu Feb 01 2024 17:18:58 GMT+0000 (Coordinated Universal Time)
# platforms=linux/amd64
#
ARG TAG=dev
FROM abcdesktopio/oc.template.alpine:$TAG
USER root
RUN apk add --no-cache --update powershell konsole font-adobe-source-code-pro
LABEL oc.icon="powershell.svg"
LABEL oc.icondata=""
LABEL oc.keyword="powershell,powershell"
LABEL oc.cat="utilities"
LABEL oc.launch="powershell.konsole"
LABEL oc.template="abcdesktopio/oc.template.alpine"
ENV ARGS="--name powershell -e /usr/bin/pwsh"
LABEL oc.name="powershell"
LABEL oc.displayname="Powershell"
LABEL oc.path="/usr/bin/konsole"
LABEL oc.type=app
LABEL oc.fileextensions="ps1;"
LABEL oc.rules="{\"homedir\":{\"default\":true}}"
LABEL oc.acl="{\"permit\":[\"all\"]}"
ENV APPNAME "powershell"
ENV APPBIN "/usr/bin/konsole"
LABEL oc.args="--name powershell -e /usr/bin/pwsh"
USER root
RUN mkdir -p /var/secrets/abcdesktop/localaccount
RUN for f in passwd shadow group gshadow ; do if [ -f /etc/$f ] ; then  cp /etc/$f /var/secrets/abcdesktop/localaccount; rm -f /etc/$f; ln -s /var/secrets/abcdesktop/localaccount/$f /etc/$f; fi; done
USER balloon
CMD [ "/composer/appli-docker-entrypoint.sh" ]

```

## Rebuild the image manually

### Download the Dockerfile manually
[Dockerfile for application powershell](https://raw.githubusercontent.com/abcdesktopio/oc.apps/main/powershell.d)
``` sh
wget https://raw.githubusercontent.com/abcdesktopio/oc.apps/main/powershell.d
```

### build the `Dockerfile` to create a container image

``` sh
docker build --build-arg TAG=main -f powershell.d -t powershell .
```

### Install the new image
>If you are using `containerd` as container runtime, use the ctr command line

 
>If you are not running this bash command on your abcdesktop node
>Replace the **ABCHOST** variable set to localhost by default to your own server ip address


``` sh
ABCHOST=localhost
docker inspect powershell > powershell.json
docker image save powershell -o powershell.tar
ctr -n k8s.io images import powershell.tar
curl -X PUT -H 'Content-Type: text/javascript' http://$ABCHOST:30443/API/manager/image -d @powershell.json

```

