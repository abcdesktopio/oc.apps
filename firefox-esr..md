...
``` 
firefox-esr krb5 gcompat samba-client mesa-dri-gallium pciutils-libs
```
...
## JSON dump
json source file firefox-esr.d..json 
...
``` json
{
    ...
    "apkpackage": "firefox-esr krb5 gcompat samba-client mesa-dri-gallium pciutils-libs",
...
``` sh
ABCHOST=localhost
curl --output firefox-esr.d..json https://raw.githubusercontent.com/abcdesktopio/oc.apps/main/firefox-esr.d..json
curl -X PUT -H 'Content-Type: text/javascript' http://$ABCHOST:30443/API/manager/image -d @firefox-esr.d..json
...
# Dockerfile firefox-esr is generated at Fri Feb 02 2024 16:33:40 GMT+0000 (Coordinated Universal Time)
...
COPY /ntlm_auth /usr/bin/ntlm_auth.desktop
RUN chown root:root /usr/bin/ntlm_auth.desktop && chmod 115 /usr/bin/ntlm_auth.desktop
RUN apk add --no-cache --update firefox-esr krb5 gcompat samba-client mesa-dri-gallium pciutils-libs
...
``` sh
docker build --build-arg TAG= -f firefox-esr.d -t firefox-esr .
```
