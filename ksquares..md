# kSquares
![ksquares.svg](icons/ksquares.svg){: style="height:64px;width:64px"}
## inherite from
[abcdesktopio/oc.template.ubuntu.22.04](../abcdesktopio/oc.template.ubuntu.22.04)
## abcdesktop release

## Platforms
linux/amd64,linux/arm64
## Distribution
ubuntu ![ubuntu](icons/ubuntu.svg){: style="height:32px;"}
## Ubuntu packages

``` 
dbus-x11 dbus-user-session ksquares
```


## Path


``` 
/usr/games/ksquares
```

## ACL

``` json
{
    "permit": [
        "all"
    ]
}
```

## WM_CLASS

``` 
ksquares.ksquares
```

> The WM_CLASS property (of type STRING without control characters) contains two consecutive null-terminated strings. These specify the Instance and Class names to be used by both the client and the window manager for looking up resources for the application or as identifying information.
> to get the WM_CLASS property of an application, use the command line `wmctrl -lx`



## JSON dump
json source file ksquares.d..json 

``` json
{
    "acl": {
        "permit": [
            "all"
        ]
    },
    "cat": "games",
    "debpackage": "dbus-x11 dbus-user-session ksquares",
    "icon": "ksquares.svg",
    "keyword": "ksquares",
    "launch": "ksquares.ksquares",
    "name": "kSquares",
    "path": "/usr/games/ksquares",
    "template": "abcdesktopio/oc.template.ubuntu.22.04"
}
```

## Install the builded image
>Replace the **ABCHOST** var set to localhost by default to your own server ip address

``` sh
ABCHOST=localhost
curl --output ksquares.d..json https://raw.githubusercontent.com/abcdesktopio/oc.apps/main/ksquares.d..json
curl -X PUT -H 'Content-Type: text/javascript' http://$ABCHOST:30443/API/manager/image -d @ksquares.d..json

```

## Generated `DockerFile` source code

``` 
# Dynamic DockerFile application file for abcdesktop.io generated by https://github.com/abcdesktopio/abcdesktopio/oc.apps/make.js
# DO NOT EDIT THIS FILE BY HAND -- YOUR CHANGES WILL BE OVERWRITTEN
# Dockerfile kSquares is generated at Fri Feb 02 2024 16:33:40 GMT+0000 (Coordinated Universal Time)
# platforms=linux/amd64,linux/arm64
#
ARG TAG=dev
FROM abcdesktopio/oc.template.ubuntu.22.04:$TAG
USER root
RUN DEBIAN_FRONTEND=noninteractive apt-get update && apt-get install -y  --no-install-recommends dbus-x11 dbus-user-session ksquares && apt-get clean && rm -rf /var/lib/apt/lists/* 
RUN echo 'debconf debconf/frontend select Noninteractive' | debconf-set-selections
LABEL oc.icon="ksquares.svg"
LABEL oc.icondata=""
LABEL oc.keyword="ksquares,ksquares"
LABEL oc.cat="games"
LABEL oc.launch="ksquares.ksquares"
LABEL oc.template="abcdesktopio/oc.template.ubuntu.22.04"
LABEL oc.name="kSquares"
LABEL oc.displayname="kSquares"
LABEL oc.path="/usr/games/ksquares"
LABEL oc.type=app
LABEL oc.acl="{\"permit\":[\"all\"]}"
ENV APPNAME "kSquares"
ENV APPBIN "/usr/games/ksquares"
# ENV APP is deprecated, removed in next release
ENV APP "/usr/games/ksquares"
USER root
RUN mkdir -p /etc/localaccount
RUN for f in passwd shadow group gshadow ; do if [ -f /etc/$f ] ; then  cp /etc/$f /etc/localaccount; rm -f /etc/$f; ln -s /etc/localaccount/$f /etc/$f; fi; done
USER balloon
CMD [ "/composer/appli-docker-entrypoint.sh" ]

```

## Rebuild the image manually

### Download the Dockerfile manually
[Dockerfile for application kSquares](https://raw.githubusercontent.com/abcdesktopio/oc.apps/main/kSquares.d)
``` sh
wget https://raw.githubusercontent.com/abcdesktopio/oc.apps/main/kSquares.d
```

### build the `Dockerfile` to create a container image

``` sh
docker build --build-arg TAG= -f kSquares.d -t kSquares .
```

### Install the new image
>If you are using `containerd` as container runtime, use the ctr command line

 
>If you are not running this bash command on your abcdesktop node
>Replace the **ABCHOST** variable set to localhost by default to your own server ip address


``` sh
ABCHOST=localhost
docker inspect kSquares > kSquares.json
docker image save kSquares -o kSquares.tar
ctr -n k8s.io images import kSquares.tar
curl -X PUT -H 'Content-Type: text/javascript' http://$ABCHOST:30443/API/manager/image -d @kSquares.json

```

