...
## JSON dump
json source file stress.d..json 
...
``` json
{
    ...
    "cat": "development",
    "debpackage": "konsole stress",
    "icon": "stress.svg",
    "keyword": "stress,cpu,shell",
    "launch": "stress.konsole",
    "name": "stress",
...
``` sh
ABCHOST=localhost
curl --output stress.d..json https://raw.githubusercontent.com/abcdesktopio/oc.apps/main/stress.d..json
curl -X PUT -H 'Content-Type: text/javascript' http://$ABCHOST:30443/API/manager/image -d @stress.d..json
...
# Dockerfile stress is generated at Mon Sep 18 2023 09:33:18 GMT+0000 (Coordinated Universal Time)
# platforms=linux/amd64,linux/arm64
#
ARG TAG=dev
FROM abcdesktopio/oc.template.ubuntu.20.04:$TAG
USER root
RUN DEBIAN_FRONTEND=noninteractive apt-get update && apt-get install -y  --no-install-recommends konsole stress && apt-get clean
...
``` sh
docker build --build-arg TAG= -f stress.d -t stress .
```
