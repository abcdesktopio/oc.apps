...
``` sh
ABCHOST=localhost
curl --output cuda.d..json https://raw.githubusercontent.com/abcdesktopio/oc.apps/main/cuda.d..json
curl -X PUT -H 'Content-Type: text/javascript' http://$ABCHOST:30443/API/manager/image -d @cuda.d..json
...
# Dockerfile cuda is generated at Mon Sep 18 2023 09:33:18 GMT+0000 (Coordinated Universal Time)
...
``` sh
docker build --build-arg TAG= -f cuda.d -t cuda .
```
