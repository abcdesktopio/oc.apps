...
``` 
ubuntu-restricted-extras firefox krb5-user samba-common-bin pulseaudio-utils
```
...
## JSON dump
json source file firefox_pod.d.3.0.json 
...
``` json
{
    ...
    "debpackage": "ubuntu-restricted-extras firefox krb5-user samba-common-bin pulseaudio-utils",
...
``` sh
ABCHOST=localhost
curl --output firefox_pod.d.3.0.json https://raw.githubusercontent.com/abcdesktopio/oc.apps/main/firefox_pod.d.3.0.json
curl -X PUT -H 'Content-Type: text/javascript' http://$ABCHOST:30443/API/manager/image -d @firefox_pod.d.3.0.json
...
# Dockerfile Firefox_pod is generated at Mon Sep 18 2023 08:19:35 GMT+0000 (Coordinated Universal Time)
# platforms=linux/amd64,linux/arm64
#
ARG TAG=dev
FROM abcdesktopio/oc.template.ubuntu.20.04:$TAG
USER root
RUN DEBIAN_FRONTEND=noninteractive apt-get update && apt-get install -y  --no-install-recommends ubuntu-restricted-extras firefox krb5-user samba-common-bin pulseaudio-utils && apt-get clean
...
``` sh
docker build --build-arg TAG=3.0 -f Firefox_pod.d -t Firefox_pod .
```
