...
## JSON dump
json source file 2048-ubuntu.d..json 
...
``` sh
ABCHOST=localhost
curl --output 2048-ubuntu.d..json https://raw.githubusercontent.com/abcdesktopio/oc.apps/main/2048-ubuntu.d..json
curl -X PUT -H 'Content-Type: text/javascript' http://$ABCHOST:30443/API/manager/image -d @2048-ubuntu.d..json
...
# Dockerfile 2048-ubuntu is generated at Thu Dec 28 2023 15:55:23 GMT+0000 (Coordinated Universal Time)
# platforms=linux/amd64,linux/arm64
#
ARG TAG=dev
FROM abcdesktopio/oc.template.ubuntu.minimal.22.04:$TAG
USER root
RUN DEBIAN_FRONTEND=noninteractive apt-get update && apt-get install -y  --no-install-recommends 2048-qt && apt-get clean && rm -rf /var/lib/apt/lists/* 
...
``` sh
docker build --build-arg TAG= -f 2048-ubuntu.d -t 2048-ubuntu .
```
