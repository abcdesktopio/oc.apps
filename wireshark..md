...
## JSON dump
json source file wireshark.d..json 
...
``` sh
ABCHOST=localhost
curl --output wireshark.d..json https://raw.githubusercontent.com/abcdesktopio/oc.apps/main/wireshark.d..json
curl -X PUT -H 'Content-Type: text/javascript' http://$ABCHOST:30443/API/manager/image -d @wireshark.d..json
...
# Dockerfile wireshark is generated at Wed Jan 31 2024 16:43:09 GMT+0000 (Coordinated Universal Time)
...
``` sh
docker build --build-arg TAG= -f wireshark.d -t wireshark .
```
