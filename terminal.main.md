...
``` 
konsole sudo bash openssh-client telnet netcat sshcommand sshfs ftp-ssl wput curl wget tftp ncftp git git-ftp ftp dbus-x11
```
...
## JSON dump
json source file terminal.d.main.json 
...
``` json
{
    ...
    "cat": "development",
    "debpackage": "konsole sudo bash openssh-client telnet netcat sshcommand sshfs ftp-ssl wput curl wget tftp ncftp git git-ftp ftp dbus-x11",
    "icon": "pantheon-terminal-icons.svg",
    "keyword": "konsole,terminal,bash,shell,cmd,admin,ftp,telnet,netcat,sshfs,curl,wget,git,ssh",
...
``` sh
ABCHOST=localhost
curl --output terminal.d.main.json https://raw.githubusercontent.com/abcdesktopio/oc.apps/main/terminal.d.main.json
curl -X PUT -H 'Content-Type: text/javascript' http://$ABCHOST:30443/API/manager/image -d @terminal.d.main.json
...
# Dockerfile Terminal is generated at Thu Aug 10 2023 12:52:07 GMT+0000 (Coordinated Universal Time)
# platforms=linux/amd64,linux/arm64
#
ARG TAG=dev
FROM abcdesktopio/oc.template.ubuntu.minimal.22.04:$TAG
USER root
RUN DEBIAN_FRONTEND=noninteractive apt-get update && apt-get install -y  --no-install-recommends konsole sudo bash openssh-client telnet netcat sshcommand sshfs ftp-ssl wput curl wget tftp ncftp git git-ftp ftp dbus-x11 && apt-get clean
...
``` sh
docker build --build-arg TAG=main -f Terminal.d -t Terminal .
```
