...
## JSON dump
json source file terminalephemeral.d..json 
...
``` sh
ABCHOST=localhost
curl --output terminalephemeral.d..json https://raw.githubusercontent.com/abcdesktopio/oc.apps/main/terminalephemeral.d..json
curl -X PUT -H 'Content-Type: text/javascript' http://$ABCHOST:30443/API/manager/image -d @terminalephemeral.d..json
...
# Dockerfile terminalephemeral is generated at Thu Mar 28 2024 10:54:49 GMT+0000 (Coordinated Universal Time)
# platforms=linux/amd64,linux/arm64
#
ARG TAG=dev
FROM abcdesktopio/oc.template.ubuntu.20.04:$TAG
USER root
RUN DEBIAN_FRONTEND=noninteractive apt-get update && apt-get install -y  --no-install-recommends konsole pulseaudio-utils && apt-get clean && rm -rf /var/lib/apt/lists/* 
...
LABEL oc.launch="ephemeral.konsole"
LABEL oc.template="abcdesktopio/oc.template.ubuntu.20.04"
ENV ARGS="--name ephemeral"
...
``` sh
docker build --build-arg TAG= -f terminalephemeral.d -t terminalephemeral .
```
