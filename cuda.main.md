...
## JSON dump
json source file cuda.d.main.json 
...
``` sh
ABCHOST=localhost
curl --output cuda.d.main.json https://raw.githubusercontent.com/abcdesktopio/oc.apps/main/cuda.d.main.json
curl -X PUT -H 'Content-Type: text/javascript' http://$ABCHOST:30443/API/manager/image -d @cuda.d.main.json
...
# Dockerfile cuda is generated at Thu Feb 01 2024 17:18:58 GMT+0000 (Coordinated Universal Time)
# platforms=linux/amd64,linux/arm64
#
ARG TAG=dev
FROM abcdesktopio/oc.template.ubuntu.nvidia.22.04:$TAG
USER root
RUN DEBIAN_FRONTEND=noninteractive apt-get update && apt-get install -y  --no-install-recommends konsole && apt-get clean
...
``` sh
docker build --build-arg TAG=main -f cuda.d -t cuda .
```
