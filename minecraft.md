...
``` 
libflite1 openjdk-8-jre at-spi2-core dbus-x11 orca libsecret-1-0
```
...
## JSON dump
json source file minecraft.d.main.json 
...
``` json
{
    ...
    "debpackage": "libflite1 openjdk-8-jre at-spi2-core dbus-x11 orca libsecret-1-0",
...
``` sh
ABCHOST=localhost
curl --output minecraft.d.main.json https://raw.githubusercontent.com/abcdesktopio/oc.apps/main/minecraft.d.main.json
curl -X PUT -H 'Content-Type: text/javascript' http://$ABCHOST:30443/API/manager/image -d @minecraft.d.main.json
...
# Dockerfile minecraft is generated at Wed Aug 02 2023 12:39:17 GMT+0000 (Coordinated Universal Time)
# platforms=linux/amd64
#
ARG TAG=dev
...
FROM abcdesktopio/oc.template.ubuntu.gtk:$TAG
USER root
RUN DEBIAN_FRONTEND=noninteractive apt-get update && apt-get install -y  --no-install-recommends libflite1 openjdk-8-jre at-spi2-core dbus-x11 orca libsecret-1-0 && apt-get clean
...
``` sh
docker build --build-arg TAG=main -f minecraft.d -t minecraft .
```
