# cudademosuite
![nvidia.svg](icons/nvidia.svg){: style="height:64px;width:64px"}
## inherite from
[abcdesktopio/oc.template.ubuntu.nvidia.22.04](../abcdesktopio/oc.template.ubuntu.nvidia.22.04)
## abcdesktop release
main
## Platforms
linux/amd64
## Distribution
ubuntu ![ubuntu](icons/ubuntu.svg){: style="height:32px;"}

``` 
PRETTY_NAME="Ubuntu 22.04.2 LTS"
NAME="Ubuntu"
VERSION_ID="22.04"
VERSION="22.04.2 LTS (Jammy Jellyfish)"
VERSION_CODENAME=jammy
ID=ubuntu
ID_LIKE=debian
HOME_URL="https://www.ubuntu.com/"
SUPPORT_URL="https://help.ubuntu.com/"
BUG_REPORT_URL="https://bugs.launchpad.net/ubuntu/"
PRIVACY_POLICY_URL="https://www.ubuntu.com/legal/terms-and-policies/privacy-policy"
UBUNTU_CODENAME=jammy

```


## Ubuntu packages

``` 
konsole git libglu1-mesa libxi6 libxinerama1 libxmu6 libglu1-mesa mesa-utils freeglut3 x11-xserver-utils cuda-demo-suite-12-0
```

## Arguments
`"--name cudademosuite"`

## Displayname


``` 
cuda demo suite
```

## Path


``` 
/usr/bin/konsole
```

## ACL

``` json
{
    "permit": [
        "all"
    ]
}
```

## WM_CLASS

``` 
cudademosuite.konsole
```

> The WM_CLASS property (of type STRING without control characters) contains two consecutive null-terminated strings. These specify the Instance and Class names to be used by both the client and the window manager for looking up resources for the application or as identifying information.
> to get the WM_CLASS property of an application, use the command line `wmctrl -lx`

## Desktopfile

``` 
/usr/share/applications/org.kde.konsole.desktop
```

> A .desktop file is a simple text file that holds information about a program. It is usually placed in “/usr/share/applications/”.

## PRE run command

> PRE run command are run **before** the package install command

```
```



## JSON dump
json source file cudademosuite.d.main.json 

``` json
{
    "acl": {
        "permit": [
            "all"
        ]
    },
    "cat": "development",
    "platforms": "linux/amd64",
    "preruncommands": [],
    "debpackage": "konsole git libglu1-mesa libxi6 libxinerama1 libxmu6 libglu1-mesa mesa-utils freeglut3 x11-xserver-utils cuda-demo-suite-12-0",
    "icon": "nvidia.svg",
    "keyword": "cuda nvidia demo suite",
    "launch": "cudademosuite.konsole",
    "name": "cudademosuite",
    "displayname": "cuda demo suite",
    "path": "/usr/bin/konsole",
    "args": "--name cudademosuite",
    "rules": {
        "homedir": {
            "default": true
        }
    },
    "template": "abcdesktopio/oc.template.ubuntu.nvidia.22.04",
    "desktopfile": "/usr/share/applications/org.kde.konsole.desktop",
    "abcdesktop_release": 3
}
```

## Install the builded image
>Replace the **ABCHOST** var set to localhost by default to your own server ip address

``` sh
ABCHOST=localhost
curl --output cudademosuite.d.main.json https://raw.githubusercontent.com/abcdesktopio/oc.apps/main/cudademosuite.d.main.json
curl -X PUT -H 'Content-Type: text/javascript' http://$ABCHOST:30443/API/manager/image -d @cudademosuite.d.main.json

```

## Generated `DockerFile` source code

``` 
# Dynamic DockerFile application file for abcdesktop.io generated by https://github.com/abcdesktopio/abcdesktopio/oc.apps/make.js
# DO NOT EDIT THIS FILE BY HAND -- YOUR CHANGES WILL BE OVERWRITTEN
# Dockerfile cudademosuite is generated at Thu Feb 01 2024 17:18:58 GMT+0000 (Coordinated Universal Time)
# platforms=linux/amd64
#
ARG TAG=dev
FROM abcdesktopio/oc.template.ubuntu.nvidia.22.04:$TAG
USER root
RUN DEBIAN_FRONTEND=noninteractive apt-get update && apt-get install -y  --no-install-recommends konsole git libglu1-mesa libxi6 libxinerama1 libxmu6 libglu1-mesa mesa-utils freeglut3 x11-xserver-utils cuda-demo-suite-12-0 && apt-get clean
RUN echo 'debconf debconf/frontend select Noninteractive' | debconf-set-selections
LABEL oc.icon="nvidia.svg"
LABEL oc.icondata=""
LABEL oc.keyword="cudademosuite,cuda nvidia demo suite"
LABEL oc.cat="development"
LABEL oc.desktopfile="org.kde.konsole.desktop"
LABEL oc.launch="cudademosuite.konsole"
LABEL oc.template="abcdesktopio/oc.template.ubuntu.nvidia.22.04"
ENV ARGS="--name cudademosuite"
LABEL oc.name="cudademosuite"
LABEL oc.displayname="cuda demo suite"
LABEL oc.path="/usr/bin/konsole"
LABEL oc.type=app
LABEL oc.rules="{\"homedir\":{\"default\":true}}"
LABEL oc.acl="{\"permit\":[\"all\"]}"
ENV APPNAME "cudademosuite"
ENV APPBIN "/usr/bin/konsole"
LABEL oc.args="--name cudademosuite"
USER root
RUN mkdir -p /var/secrets/abcdesktop/localaccount
RUN for f in passwd shadow group gshadow ; do if [ -f /etc/$f ] ; then  cp /etc/$f /var/secrets/abcdesktop/localaccount; rm -f /etc/$f; ln -s /var/secrets/abcdesktop/localaccount/$f /etc/$f; fi; done
USER balloon
CMD [ "/composer/appli-docker-entrypoint.sh" ]

```

## Rebuild the image manually

### Download the Dockerfile manually
[Dockerfile for application cudademosuite](https://raw.githubusercontent.com/abcdesktopio/oc.apps/main/cudademosuite.d)
``` sh
wget https://raw.githubusercontent.com/abcdesktopio/oc.apps/main/cudademosuite.d
```

### build the `Dockerfile` to create a container image

``` sh
docker build --build-arg TAG=main -f cudademosuite.d -t cudademosuite .
```

### Install the new image
>If you are using `containerd` as container runtime, use the ctr command line

 
>If you are not running this bash command on your abcdesktop node
>Replace the **ABCHOST** variable set to localhost by default to your own server ip address


``` sh
ABCHOST=localhost
docker inspect cudademosuite > cudademosuite.json
docker image save cudademosuite -o cudademosuite.tar
ctr -n k8s.io images import cudademosuite.tar
curl -X PUT -H 'Content-Type: text/javascript' http://$ABCHOST:30443/API/manager/image -d @cudademosuite.json

```

