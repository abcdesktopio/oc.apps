...
## JSON dump
json source file corsix-th.d..json 
...
``` sh
ABCHOST=localhost
curl --output corsix-th.d..json https://raw.githubusercontent.com/abcdesktopio/oc.apps/main/corsix-th.d..json
curl -X PUT -H 'Content-Type: text/javascript' http://$ABCHOST:30443/API/manager/image -d @corsix-th.d..json
...
# Dockerfile corsix-th is generated at Fri Feb 02 2024 14:47:39 GMT+0000 (Coordinated Universal Time)
...
RUN cd /composer &&  curl -Ls https://th.corsix.org/Demo.zip -o Demo.zip && unzip Demo.zip && rm -rf Demo.zip
COPY corsix-th.config.txt /composer/corsix-th.config.txt
RUN DEBIAN_FRONTEND=noninteractive apt-get update && apt-get install -y  --no-install-recommends libgl1 corsix-th && apt-get clean && rm -rf /var/lib/apt/lists/* 
...
``` sh
docker build --build-arg TAG= -f corsix-th.d -t corsix-th .
```
