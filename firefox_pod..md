...
## inherite from
[abcdesktopio/oc.template.ubuntu.20.04](../abcdesktopio/oc.template.ubuntu.20.04)
## abcdesktop release
...
``` 
ubuntu-restricted-extras firefox krb5-user samba-common-bin pulseaudio-utils firefox-locale-fr firefox-locale-en firefox-locale-ro firefox-locale-de
```

## Arguments
`"--class firefoxubuntupod"`
...
## JSON dump
json source file firefox_pod.d..json 
...
``` json
{
    ...
    "debpackage": "ubuntu-restricted-extras firefox krb5-user samba-common-bin pulseaudio-utils firefox-locale-fr firefox-locale-en firefox-locale-ro firefox-locale-de",
    "icon": "circle_firefox.svg",
    "keyword": "firefox,mozilla,web,internet",
    "launch": "Navigator.firefoxubuntupod",
    "name": "Firefox_pod",
    "displayname": "Firefox_pod (ubuntu)",
    "showinview": "dock",
    "args": "--class firefoxubuntupod",
...
``` sh
ABCHOST=localhost
curl --output firefox_pod.d..json https://raw.githubusercontent.com/abcdesktopio/oc.apps/main/firefox_pod.d..json
curl -X PUT -H 'Content-Type: text/javascript' http://$ABCHOST:30443/API/manager/image -d @firefox_pod.d..json
...
# Dockerfile Firefox_pod is generated at Thu Mar 28 2024 20:47:16 GMT+0000 (Coordinated Universal Time)
...
RUN DEBIAN_FRONTEND=noninteractive apt-get update && apt-get install -y  --no-install-recommends ubuntu-restricted-extras firefox krb5-user samba-common-bin pulseaudio-utils firefox-locale-fr firefox-locale-en firefox-locale-ro firefox-locale-de && apt-get clean && rm -rf /var/lib/apt/lists/* 
...
LABEL oc.keyword="firefox_pod,firefox,mozilla,web,internet"
LABEL oc.cat="office"
LABEL oc.desktopfile="firefox.desktop"
LABEL oc.launch="Navigator.firefoxubuntupod"
LABEL oc.template="abcdesktopio/oc.template.ubuntu.20.04"
ENV ARGS="--class firefoxubuntupod"
LABEL oc.name="Firefox_pod"
LABEL oc.displayname="Firefox_pod (ubuntu)"
...
ENV APPNAME "Firefox_pod"
ENV APPBIN "/usr/bin/firefox"
# ENV APP is deprecated, removed in next release
ENV APP "/usr/bin/firefox"
LABEL oc.args="--class firefoxubuntupod"
...
[Dockerfile for application Firefox_pod](https://raw.githubusercontent.com/abcdesktopio/oc.apps/main/Firefox_pod.d)
``` sh
wget https://raw.githubusercontent.com/abcdesktopio/oc.apps/main/Firefox_pod.d
```

### build the `Dockerfile` to create a container image

``` sh
docker build --build-arg TAG= -f Firefox_pod.d -t Firefox_pod .
```
...
``` sh
ABCHOST=localhost
docker inspect Firefox_pod > Firefox_pod.json
docker image save Firefox_pod -o Firefox_pod.tar
ctr -n k8s.io images import Firefox_pod.tar
curl -X PUT -H 'Content-Type: text/javascript' http://$ABCHOST:30443/API/manager/image -d @Firefox_pod.json
