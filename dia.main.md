# Dia
![circle_dia.svg](icons/circle_dia.svg){: style="height:64px;width:64px"}
## inherite from
[abcdesktopio/oc.template.alpine.3.17](../abcdesktopio/oc.template.alpine.3.17)
## abcdesktop release
main
## Platforms
linux/amd64,linux/arm64
## Distribution
alpine ![alpine](icons/alpine.svg){: style="height:32px;"}

``` 
NAME="Alpine Linux"
ID=alpine
VERSION_ID=3.17.4
PRETTY_NAME="Alpine Linux v3.17"
HOME_URL="https://alpinelinux.org/"
BUG_REPORT_URL="https://gitlab.alpinelinux.org/alpine/aports/-/issues"

```


## Alpine packages

``` 
dia dia-lang
```


## Path


``` 
/usr/bin/dia
```

## Mimetype

``` 
application/x-dia-diagram;
```

## File extensions
`"dia"`
## Legacy file extensions
`"dia"`
## ACL

``` json
{
    "permit": [
        "all"
    ]
}
```

## WM_CLASS

``` 
dia-gnome.Dia-gnome
```

> The WM_CLASS property (of type STRING without control characters) contains two consecutive null-terminated strings. These specify the Instance and Class names to be used by both the client and the window manager for looking up resources for the application or as identifying information.
> to get the WM_CLASS property of an application, use the command line `wmctrl -lx`

## Desktopfile

``` 
/usr/share/applications/dia.desktop
```

> A .desktop file is a simple text file that holds information about a program. It is usually placed in “/usr/share/applications/”.



## JSON dump
json source file dia.d.main.json 

``` json
{
    "acl": {
        "permit": [
            "all"
        ]
    },
    "cat": "graphics",
    "apkpackage": "dia dia-lang",
    "installrecommends": true,
    "icon": "circle_dia.svg",
    "launch": "dia-gnome.Dia-gnome",
    "name": "Dia",
    "rules": {
        "homedir": {
            "default": true
        }
    },
    "path": "/usr/bin/dia",
    "template": "abcdesktopio/oc.template.alpine.3.17",
    "mimetype": "application/x-dia-diagram;",
    "fileextensions": "dia",
    "legacyfileextensions": "dia",
    "desktopfile": "/usr/share/applications/dia.desktop"
}
```

## Install the builded image
>Replace the **ABCHOST** var set to localhost by default to your own server ip address

``` sh
ABCHOST=localhost
curl --output dia.d.main.json https://raw.githubusercontent.com/abcdesktopio/oc.apps/main/dia.d.main.json
curl -X PUT -H 'Content-Type: text/javascript' http://$ABCHOST:30443/API/manager/image -d @dia.d.main.json

```

## Generated `DockerFile` source code

``` 
# Dynamic DockerFile application file for abcdesktop.io generated by https://github.com/abcdesktopio/abcdesktopio/oc.apps/make.js
# DO NOT EDIT THIS FILE BY HAND -- YOUR CHANGES WILL BE OVERWRITTEN
# Dockerfile Dia is generated at Thu Aug 10 2023 12:52:07 GMT+0000 (Coordinated Universal Time)
# platforms=linux/amd64,linux/arm64
#
ARG TAG=dev
FROM abcdesktopio/oc.template.alpine.3.17:$TAG
USER root
RUN apk add --no-cache --update dia dia-lang
LABEL oc.icon="circle_dia.svg"
LABEL oc.icondata=""
LABEL oc.keyword="dia"
LABEL oc.cat="graphics"
LABEL oc.desktopfile="dia.desktop"
LABEL oc.launch="dia-gnome.Dia-gnome"
LABEL oc.template="abcdesktopio/oc.template.alpine.3.17"
LABEL oc.name="Dia"
LABEL oc.displayname="Dia"
LABEL oc.path="/usr/bin/dia"
LABEL oc.type=app
LABEL oc.mimetype="application/x-dia-diagram;"
LABEL oc.fileextensions="dia"
LABEL oc.legacyfileextensions="dia"
LABEL oc.rules="{\"homedir\":{\"default\":true}}"
LABEL oc.acl="{\"permit\":[\"all\"]}"
ENV APPNAME "Dia"
ENV APPBIN "/usr/bin/dia"
USER root
RUN mkdir -p /var/secrets/abcdesktop/localaccount
RUN for f in passwd shadow group gshadow ; do if [ -f /etc/$f ] ; then  cp /etc/$f /var/secrets/abcdesktop/localaccount; rm -f /etc/$f; ln -s /var/secrets/abcdesktop/localaccount/$f /etc/$f; fi; done
USER balloon
CMD [ "/composer/appli-docker-entrypoint.sh" ]

```

## Rebuild the image manually

### Download the Dockerfile manually
[Dockerfile for application Dia](https://raw.githubusercontent.com/abcdesktopio/oc.apps/main/Dia.d)
``` sh
wget https://raw.githubusercontent.com/abcdesktopio/oc.apps/main/Dia.d
```

### build the `Dockerfile` to create a container image

``` sh
docker build --build-arg TAG=main -f Dia.d -t Dia .
```

### Install the new image
>If you are using `containerd` as container runtime, use the ctr command line

 
>If you are not running this bash command on your abcdesktop node
>Replace the **ABCHOST** variable set to localhost by default to your own server ip address


``` sh
ABCHOST=localhost
docker inspect Dia > Dia.json
docker image save Dia -o Dia.tar
ctr -n k8s.io images import Dia.tar
curl -X PUT -H 'Content-Type: text/javascript' http://$ABCHOST:30443/API/manager/image -d @Dia.json

```

