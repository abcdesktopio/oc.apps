...
``` 
sudo qterminal dbus-x11 net-tools font-opensans font-adobe-100dpi font-noto font-xfree86-type1 ttf-freefont font-ibm-type1 font-liberation
```
...
## JSON dump
json source file qterminalpod.d.3.0.json 
...
``` json
{
    ...
    "cat": "development",
    "apkpackage": "sudo qterminal dbus-x11 net-tools font-opensans font-adobe-100dpi font-noto font-xfree86-type1 ttf-freefont font-ibm-type1 font-liberation",
    "postruncommands": [
        "ENV SHELL=/bin/bash",
        "RUN echo \"ALL ALL=(ALL:ALL) ALL\">/etc/sudoers.d/all"
    ],
    "icon": "pantheon-terminal-icons.svg",
    "keyword": "pod,qterminal,shell",
    "launch": "no.qterminal",
    "name": "qterminalpod",
    "displayname": "qterminal [Pod]",
    ...
    "desktopfile": "/usr/share/applications/qterminal.desktop"
}
```
...
``` sh
ABCHOST=localhost
curl --output qterminalpod.d.3.0.json https://raw.githubusercontent.com/abcdesktopio/oc.apps/main/qterminalpod.d.3.0.json
curl -X PUT -H 'Content-Type: text/javascript' http://$ABCHOST:30443/API/manager/image -d @qterminalpod.d.3.0.json
...
# Dockerfile qterminalpod is generated at Mon Sep 18 2023 08:19:36 GMT+0000 (Coordinated Universal Time)
# platforms=linux/amd64,linux/arm64
#
ARG TAG=dev
FROM abcdesktopio/oc.template.alpine.minimal:$TAG
USER root
RUN apk add --no-cache --update sudo qterminal dbus-x11 net-tools font-opensans font-adobe-100dpi font-noto font-xfree86-type1 ttf-freefont font-ibm-type1 font-liberation
...
LABEL oc.keyword="qterminalpod,pod,qterminal,shell"
LABEL oc.cat="development"
LABEL oc.desktopfile="qterminal.desktop"
LABEL oc.launch="no.qterminal"
LABEL oc.template="abcdesktopio/oc.template.alpine.minimal"
LABEL oc.name="qterminalpod"
LABEL oc.displayname="qterminal [Pod]"
LABEL oc.path="/usr/bin/qterminal"
LABEL oc.type=app
LABEL oc.rules="{\"homedir\":{\"default\":false}}"
LABEL oc.acl="{\"permit\":[\"all\"]}"
ENV APPNAME "qterminalpod"
ENV APPBIN "/usr/bin/qterminal"
# ENV APP is deprecated, removed in next release
ENV APP "/usr/bin/qterminal"
...
[Dockerfile for application qterminalpod](https://raw.githubusercontent.com/abcdesktopio/oc.apps/main/qterminalpod.d)
``` sh
wget https://raw.githubusercontent.com/abcdesktopio/oc.apps/main/qterminalpod.d
```

### build the `Dockerfile` to create a container image

``` sh
docker build --build-arg TAG=3.0 -f qterminalpod.d -t qterminalpod .
```
...
``` sh
ABCHOST=localhost
docker inspect qterminalpod > qterminalpod.json
docker image save qterminalpod -o qterminalpod.tar
ctr -n k8s.io images import qterminalpod.tar
curl -X PUT -H 'Content-Type: text/javascript' http://$ABCHOST:30443/API/manager/image -d @qterminalpod.json
