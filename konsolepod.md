...
## JSON dump
json source file konsolepod.d.main.json 
...
``` sh
ABCHOST=localhost
curl --output konsolepod.d.main.json https://raw.githubusercontent.com/abcdesktopio/oc.apps/main/konsolepod.d.main.json
curl -X PUT -H 'Content-Type: text/javascript' http://$ABCHOST:30443/API/manager/image -d @konsolepod.d.main.json
...
# Dockerfile konsolepod is generated at Wed Aug 02 2023 12:39:17 GMT+0000 (Coordinated Universal Time)
...
LABEL oc.keyword="konsolepod,ksonsole,console,shell,bash,sh"
LABEL oc.cat="development"
LABEL oc.launch="pod.konsole"
LABEL oc.template="abcdesktopio/oc.template.alpine"
ENV ARGS="--name pod"
LABEL oc.name="konsolepod"
LABEL oc.displayname="konsole [pod]"
LABEL oc.path="/usr/bin/konsole"
LABEL oc.type=app
LABEL oc.rules="{\"homedir\":{\"default\":true}}"
LABEL oc.acl="{\"permit\":[\"all\"]}"
ENV APPNAME "konsolepod"
...
[Dockerfile for application konsolepod](https://raw.githubusercontent.com/abcdesktopio/oc.apps/main/konsolepod.d)
``` sh
wget https://raw.githubusercontent.com/abcdesktopio/oc.apps/main/konsolepod.d
```

### build the `Dockerfile` to create a container image

``` sh
docker build --build-arg TAG=main -f konsolepod.d -t konsolepod .
```
...
``` sh
ABCHOST=localhost
docker inspect konsolepod > konsolepod.json
docker image save konsolepod -o konsolepod.tar
ctr -n k8s.io images import konsolepod.tar
curl -X PUT -H 'Content-Type: text/javascript' http://$ABCHOST:30443/API/manager/image -d @konsolepod.json
