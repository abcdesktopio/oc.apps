...
``` 
firefox krb5 gcompat samba-client mesa-dri-gallium pciutils-libs
```
...
RUN chown root:root /usr/bin/ntlm_auth.desktop && chmod 115 /usr/bin/ntlm_auth.desktop
```



## JSON dump
json source file firefoxalpine.d.3.0.json 
...
``` json
{
    ...
    "apkpackage": "firefox krb5 gcompat samba-client mesa-dri-gallium pciutils-libs",
...
``` sh
ABCHOST=localhost
curl --output firefoxalpine.d.3.0.json https://raw.githubusercontent.com/abcdesktopio/oc.apps/main/firefoxalpine.d.3.0.json
curl -X PUT -H 'Content-Type: text/javascript' http://$ABCHOST:30443/API/manager/image -d @firefoxalpine.d.3.0.json
...
# Dockerfile FirefoxAlpine is generated at Mon Sep 18 2023 08:19:35 GMT+0000 (Coordinated Universal Time)
# platforms=linux/amd64,linux/arm64
#
ARG TAG=dev
FROM abcdesktopio/oc.template.alpine.edge.gtk:$TAG
USER root
RUN apk add --no-cache --update firefox krb5 gcompat samba-client mesa-dri-gallium pciutils-libs
...
LABEL oc.keyword="firefoxalpine,firefox,mozilla,web,internet"
LABEL oc.cat="office"
...
[Dockerfile for application FirefoxAlpine](https://raw.githubusercontent.com/abcdesktopio/oc.apps/main/FirefoxAlpine.d)
``` sh
wget https://raw.githubusercontent.com/abcdesktopio/oc.apps/main/FirefoxAlpine.d
```

### build the `Dockerfile` to create a container image

``` sh
docker build --build-arg TAG=3.0 -f FirefoxAlpine.d -t FirefoxAlpine .
```
...
``` sh
ABCHOST=localhost
docker inspect FirefoxAlpine > FirefoxAlpine.json
docker image save FirefoxAlpine -o FirefoxAlpine.tar
ctr -n k8s.io images import FirefoxAlpine.tar
curl -X PUT -H 'Content-Type: text/javascript' http://$ABCHOST:30443/API/manager/image -d @FirefoxAlpine.json
